...
```text
aws ec2 run-instances --image-id ami-0aa210fd2121a98b7 --instance-type m7i-flex.large --key-name XXX --security-group-ids sg-XXX --subnet-id subnet-XXX --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=ondia-capstone-nat-instance-dont-terminate}]' --disable-api-termination
```

- for example
```text
aws ec2 run-instances --image-id ami-0aa210fd2121a98b7 --instance-type m7i-flex.large --key-name ramazan-key --security-group-ids sg-05ce8b2d34bd29f9f --subnet-id subnet-0ed8543a7d1c9273c --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=ondia-capstone-nat-instance-dont-terminate}]' --disable-api-termination
```
...
git clone https://$TOKEN@github.com/ramznagc/capstone-project.git
